...
class Rational(n: Long, d: Long) extends Ordered[Rational] {
  ...
  def strictToInt: Int =  if(numerator % denominator != 0) throw new IllegalArgumentException("Can't execute strict conversion")
...
object Rational {
  ...
  implicit def longToRational(i:Long): Rational =new Rational(i)
}
